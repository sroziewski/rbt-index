...
int isCompressedFile(const char* filePath) {
    const char *dot = strrchr(filePath, '.');
    return dot && strcasecmp(dot, ".7z") == 0 || dot && strcasecmp(dot, ".tbz") == 0 || dot && strcasecmp(dot, ".bz2") == 0 || dot && strcasecmp(dot, ".tar") == 0 || dot && strcasecmp(dot, ".arj") == 0;
}
...
int isSqlFile(const char* filePath) {
    const char *dot = strrchr(filePath, '.');
    return dot && strcasecmp(dot, ".sql") == 0 || dot && strcasecmp(dot, ".psql") == 0 || dot && strcasecmp(dot, ".hql") == 0;
}

int isCalcFile(const char* filePath) {
    const char *dot = strrchr(filePath, '.');
    return dot && strcasecmp(dot, ".ods") == 0 || dot && strcasecmp(dot, ".xls") == 0 || dot && strcasecmp(dot, ".xlsx") == 0;
}
...
const char* getFileTypeCategory(const char *mimeType, const char* filePath) {
    ...
    if (strstr(mimeType, "image/") == mimeType) {
        return "T_IMAGE";
    } else if (strstr(mimeType, "video/") == mimeType) {
        return "T_FILM";
    } else if (strstr(mimeType, "audio/") == mimeType) {
        return "T_AUDIO";
    } else if (strstr(mimeType, "application/zip") == mimeType ||
    ...
    while (1) {
        ...
        while ((entry = readdir(dp)) != NULL) {
            ...
            if (numEntries >= entryCapacity) {
                ...
                char **newDirEntries = (char **)realloc(dirEntries, entryCapacity * sizeof(char *));
                if (!newDirEntries) {
                    perror("realloc");
                    closedir(dp);
                    free(currentPath);
                    ...
                    if (magic != NULL && magic_load(magic, NULL) == 0) {
                        ...
                        if (mimeType) {
		                    ...
		                    #pragma omp critical
		                    {
		                        ...
		                        snprintf((*entries)[current].type, sizeof((*entries)[current].type), "%s", getFileTypeCategory(mimeType, fullPath));
		                    }
		                    #pragma omp atomic
